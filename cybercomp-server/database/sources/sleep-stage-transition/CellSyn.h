...
#define I_HH    {{I_HH}}     // 1 -- HH model; 0 -- for Maps;
...
#if (I_HH==1)
#define N_DEND   {{N_DEND}}
#define N_SOMA   {{N_SOMA}} //3
#else 
#define N_DEND   0
#define N_SOMA   0
#endif
...
//other defines
#define TAU      {{TAU}} //integration time step
#define TAU_Map  {{TAU_Map}}  // time step for maps
#define TAUr     {{TAUr}}   //(TAU_Map/TAU)
...
class REsyn:public CellSyn{
 ...
 REsyn(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((RE*)base_cell)->E_l = {{RE_E_l}}; 
    ((RE*)base_cell)->G_Ca = {{RE_G_Ca}}; //2.3; 
    ((RE*)base_cell)->G_kl = {{RE_G_kl}}; //0.015; //0.005;
    this->AMPA_Rsum=0;
    this->ismap=0;
  ...
  void memory(Syn_Info **syn_info,int num_syns){
    ...
    for(i =0; i < num_syns; i++){
      ...
      if(syns[i]->to_type == E_RE){
        ((GABA_A*)syns[i])->E_GABA = {{GABA_A_E_GABA}};
      }
    } 
...
class TCcore: public CellSyn {
 ...
 TCcore(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((TC*)base_cell)->G_A = {{TCcore_G_A}};
    ((TC*)base_cell)->ginc = {{TCcore_ginc}}; // TB 1.5; //2.4; // 0.9; //2; //SWS-maxim
    //((TC*)base_cell)->E_l = {{TCcore_E_l}};
    ((TC*)base_cell)->G_Ca = {{TCcore_G_Ca}}; //2.7; // TB 2.2; //2.7; //2.3; //2.; //SWS-maxim
   
    ((TC*)base_cell)->D = {{TCcore_D}};
    ((TC*)base_cell)->pc = {{TCcore_pc}};
    ((TC*)base_cell)->k4 = {{TCcore_k4}};
    ((TC*)base_cell)->Vtr = {{TCcore_Vtr}};
    ((TC*)base_cell)->VtrK = {{TCcore_VtrK}};
    ((TC*)base_cell)->G_K = {{TCcore_G_K}};
    ((TC*)base_cell)->G_h = {{TCcore_G_h}};  
  
    ((TC*)base_cell)->G_kl = {{TCcore_G_kl}}; //0.0142;  // TB 0.03; //0.0142;  SWS-maxim 
    ((TC*)base_cell)->DC = {{TCcore_DC}}; //-0.05;
  ...
  void memory(Syn_Info **syn_info,int num_syns){
    ...
    for(i =0; i < num_syns; i++){
      ...
      if(syns[i]->type == E_AMPA){
        ...
        ((GABA_A*)syns[i])->E_GABA = {{TCcore_memory_E_GABA}}; //-85; //(J.Neur.1997,17(7),2348)
      }else if(syns[i]->type == E_GABA_B){
        /* syns[i] = new GB(syn_info[i]); */
        //}else if(syns[i]->type == E_GABA_B){
        ((GB*)syns[i])->K1 = {{TCcore_memory_GB_K1}}; //0.09;
        ((GB*)syns[i])->K2 = {{TCcore_memory_GB_K2}}; 
        ((GB*)syns[i])->K3 = {{TCcore_memory_GB_K3}}; //0.18;
        ((GB*)syns[i])->K4 = {{TCcore_memory_GB_K4}};
...
class TCmatrix: public CellSyn {
 ...
 TCmatrix(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((TC*)base_cell)->G_A = {{TCmatrix_G_A}};
    ((TC*)base_cell)->ginc = {{TCmatrix_ginc}}; // TB 1.5; //2.4; // 0.9; //2; //SWS-maxim
    //((TC*)base_cell)->E_l = {{TCmatrix_E_l}};
    ((TC*)base_cell)->G_Ca = {{TCmatrix_G_Ca}}; //2.7; // TB 2.2; //2.7; //2.3; //2.; //SWS-maxim
   
    ((TC*)base_cell)->D = {{TCmatrix_D}};
    ((TC*)base_cell)->pc = {{TCmatrix_pc}};
    ((TC*)base_cell)->k4 = {{TCmatrix_k4}};
    ((TC*)base_cell)->Vtr = {{TCmatrix_Vtr}};
    ((TC*)base_cell)->VtrK = {{TCmatrix_VtrK}};
    ((TC*)base_cell)->G_K = {{TCmatrix_G_K}};
    ((TC*)base_cell)->G_h = {{TCmatrix_G_h}};  
  
    ((TC*)base_cell)->G_kl = {{TCmatrix_G_kl}}; //0.0142;  // TB 0.03; //0.0142;  SWS-maxim 
    ((TC*)base_cell)->DC = {{TCmatrix_DC}}; //-0.05;
  ...
  void memory(Syn_Info **syn_info,int num_syns){
    ...
    for(i =0; i < num_syns; i++){
      ...
      if(syns[i]->type == E_GABA_A){
        /* syns[i] = new GABA_A(syn_info[i]); */
        ((GABA_A*)syns[i])->E_GABA = {{TCmatrix_memory_E_GABA}}; //-85; //(J.Neur.1997,17(7),2348)
      }else if(syns[i]->type == E_GABA_B){
        /*syns[i] = new GB(syn_info[i]); */
        //}else if(syns[i]->type == E_GABA_B){
        ((GB*)syns[i])->K1 = {{TCmatrix_memory_GB_K1}}; //0.09;
        ((GB*)syns[i])->K2 = {{TCmatrix_memory_GB_K2}};
        ((GB*)syns[i])->K3 = {{TCmatrix_memory_GB_K3}}; //0.18;
        ((GB*)syns[i])->K4 = {{TCmatrix_memory_GB_K4}};
        ((GB*)syns[i])->init(y,12+2*k);
        k = k + 1;
      }
...
class CXsyn: public CellSyn {
 ...
 CXsyn(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((CX*)base_cell)->G_kl = {{CXsyn_G_kl}}; //0.002; //0.0025;
    ((CX*)base_cell)->E_l = {{CXsyn_E_l}}; //-67;
    ((CX*)base_cell)->G_Km = {{CXsyn_G_Km}}; //0.020; // TB 0.01;      //0.02;    //SWS-maxim
    ((CX*)base_cell)->G_l = {{CXsyn_G_l}} + (((double) rand() / (RAND_MAX)) + 1) * 0.003;
...
class CXasyn: public CellSyn {
 ...
 CXasyn(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    this->old_v = base_cell->get_v_soma();
    memory(cell_info->syn_info,num_syns);
    ((CX*)base_cell)->G_kl = {{CXasyn_G_kl}}; //0.002; //0.0025;
    ((CX*)base_cell)->E_l = {{CXasyn_E_l}}; //-67;
    ((CX*)base_cell)->G_Km = {{CXasyn_G_Km}}; //0.020; // TB 0.01;      //0.02;    //SWS-maxim
    ((CX*)base_cell)->G_l =  {{CXasyn_G_l}}; //1.0e3/30000; //0.06; // TB 1.0e3/30000; //0.06;  //SWS-maxim
...
class CXsyn6: public CellSyn {
 ...
 CXsyn6(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((CX*)base_cell)->G_kl = {{CXsyn6_G_kl}}; //0.002; //0.0025;
    ((CX*)base_cell)->E_l = {{CXsyn6_E_l}}; //-67;
    ((CX*)base_cell)->G_Km = {{CXsyn6_G_Km}}; //0.020; // TB 0.01;      //0.02;    //SWS-maxim
    ((CX*)base_cell)->G_l =  {{CXsyn6_G_l}}; //1.0e3/30000; //0.06; // TB 1.0e3/30000; //0.06;  //SWS-maxim
    this->ismap=0;
    this->AMPA_Rsum=0;
  }
...
class INsynCore: public CellSyn {
 ...
 INsynCore(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    base_cell = new CX();
    ((CX*)base_cell)->rho = {{INsynCore_rho}};         
    ((CX*)base_cell)->CX_DEND::G_Nap = {{INsynCore_DEND_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Nap = {{INsynCore_SOMA_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Na = {{INsynCore_SOMA_G_Na}};
    //((CX*)base_cell)->E_l = {{INsynCore_E_l}}; //-67;
    //((CX*)base_cell)->G_Km = {{INsynCore_G_Km}};  

    ((CX*)base_cell)->G_kl = {{INsynCore_G_Kl}}; //0.002; //0.0025;
    ((CX*)base_cell)->G_l = {{INsynCore_G_l}} + (((double) rand() / (RAND_MAX)) + 1) * 0.003;
...
class INsynMatrix: public CellSyn {
 ...
 INsynMatrix(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    base_cell = new CX();
    ((CX*)base_cell)->rho = {{INsynMatrix_rho}};         
    ((CX*)base_cell)->CX_DEND::G_Nap = {{INsynMatrix_DEND_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Nap = {{INsynMatrix_SOMA_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Na = {{INsynMatrix_SOMA_G_Na}};
...
class INsyn6: public CellSyn {
 ...
 INsyn6(int i, int j,int b_size, int t_size,Cell_Info *cell_info,int type):CellSyn(i,j,b_size,t_size,cell_info,type){
    ...
    ((CX*)base_cell)->rho = {{INsyn6_rho}};         
    ((CX*)base_cell)->CX_DEND::G_Nap = {{INsyn6_DEND_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Nap = {{INsyn6_SOMA_G_Nap}};
    ((CX*)base_cell)->CX_SOMA::G_Na = {{INsyn6_SOMA_G_Na}};
