...
parser.add_argument('--pretrained_task2', default='', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--image_size', type=int, default=224) # image resolution, 32,64,128,224
best_acc1 = 0
best_acc5 = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    param_groups = [dict(params=classifier_parameters, lr=args.lr)]
...
def safe_load_dict(model, new_model_state, new_model_state2, should_resume_all_params=False):
    ...
    if should_resume_all_params:
        for old_name, old_param in old_model_state.items():
            assert old_name in list(new_model_state.keys()), "{} parameter is not present in resumed checkpoint".format(
                old_name)
    for name, param in new_model_state2.items():
        n = name.split('.')
        beg = n[0]
        end = n[1:]
        if beg == 'module':
    ...
    for name, param in new_model_state.items():
        ...
        if old_model_state[name].shape != param.shape:
            ...
            #if name.startswith('layer4.0.conv2.') or name.startswith('layer4.0.bn2.') or name.startswith('layer4.1.'): # or name.startswith('fc.'): # 224x224
            if name.startswith('layer4.0.') or name.startswith('layer4.1.'): # or name.startswith('fc.'): # 32x32
                print(name)
                old_model_state[name].copy_(param)
    if c == 0:
        raise AssertionError('No previous ckpt names matched and the ckpt was not loaded properly.')
